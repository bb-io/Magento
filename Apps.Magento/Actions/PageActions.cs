...
public class PageActions(InvocationContext invocationContext, IFileManagementClient fileManagementClient) : AppInvocable(invocationContext)
{
    ...
    public async Task<FileReference> GetPageByIdAsHtmlAsync([ActionParameter] PageIdentifier identifier)
    {
    ...
    {
        var request = new ApiRequest("/rest/default/V1/cmsPage", Method.Post, Creds)
            .AddBody(new
            {
                page = new
                {
